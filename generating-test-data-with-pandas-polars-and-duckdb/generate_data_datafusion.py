...
import datafusion as df_engine
import numpy as np
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate test data with DataFusion")
...
# Create PyArrow Table (DataFusion uses Arrow)
...
# Use DataFusion to write the data to CSV
ctx = df_engine.SessionContext()
df = ctx.from_arrow(table)
df.write_csv(DATA_DIR / "events_df.csv", with_header=True)
